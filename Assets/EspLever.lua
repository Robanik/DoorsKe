...
local Settings = {
    ...
    TextColor = Color3.fromRGB(255, 220, 0),
    HighlightColor = Color3.fromRGB(200, 0, 0),
    BoxThickness = 2,
    TracerThickness = 2,
    BoxTransparency = 0,
    ...
    for object, espData in pairs(ESPObjects) do
        if not object or not object.Parent then
            ...
            pcall(function()
                ...
                if espData.NameTag then
                    updateText(espData.NameTag, "⚙️ " .. espData.Name, object.Position, topLeft, bottomRight)
                end
        ...
        if not foundLever then
            for _, descendant in pairs(room:GetDescendants()) do
                if descendant:IsA("Model") then
                    ...
                    if name:find("lever") or name:find("switch") or name:find("valve") then
                        local hasESP = false
                        for obj, _ in pairs(ESPObjects) do
                            if obj and obj.Parent and obj:IsDescendantOf(descendant) then
                                hasESP = true
                                break
                            end
                        end
                ...
                if descendant:IsA("BasePart") then
                    ...
                    if name:find("lever") or name:find("switch") or name:find("valve") or 
                       path:find("leverforgate") then
                        
                        local hasESP = false
                        for obj, _ in pairs(ESPObjects) do
                            if obj and obj.Parent and obj == descendant then
                                hasESP = true
                                break
                            end
                        end
...
if currentRooms then
    currentRooms.DescendantAdded:Connect(function(descendant)
        ...
        if descendant:IsA("Model") then
            ...
            if name:find("lever") or name:find("switch") or name:find("valve") then
                local room = descendant:FindFirstAncestorOfClass("Model")
        ...
        if descendant:IsA("BasePart") then
            ...
            if name:find("lever") or name:find("switch") or name:find("valve") then
                local room = descendant:FindFirstAncestor(function(ancestor)
                    return ancestor.Parent == currentRooms
                end)
